...
public class MyTreeBenchmarks {
    ...
    public void addElements(int number) {
        List<Integer> samples = new Sampler(new Random(), number).getElements();
        long start = System.currentTimeMillis();
        for (Integer sample: samples) {
            tree.add(sample);
        }
        long stop = System.currentTimeMillis();
        //System.out.println("Tijd van toevoegen van " + number + " elementen: " + (stop - start) + " ms.");
        //System.out.println("Aantal bezochte toppen bij toevoegen van " + number + " elementen: " + tree.getNodesVisited());
    }

    public void removeElements(int number) {
        tree.setNodesVisited(0);
        long start = System.currentTimeMillis();
        List<Integer> samplesToRemove = new Sampler(new Random(), number).getElements();
        for (Integer sample: samplesToRemove) {
            tree.remove(sample);
        }
        long stop = System.currentTimeMillis();
        System.out.println("Tijd van verwijderen van " + number + " elementen: " + (stop - start) + " ms.");
        System.out.println("Aantal bezochte toppen bij verwijderen van " + number + " elementen: " + tree.getNodesVisited());
    }
